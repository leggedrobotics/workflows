    ...
    def visu(self, image, target, pred):
        ...
        for b in range(image.shape[0]):
            if self._visu_count[self._mode] < self._exp["visualizer"]["store_n"][self._mode]:
                ...
                self._visualizer.plot_image(image[b], tag=f"{self._mode}_image_{c}")
                self._visualizer.plot_segmentation(pred[b, 0], tag=f"{self._mode}_pred_{c}")
                self._visualizer.plot_segmentation(target[b], tag=f"{self._mode}_target_{c}")

                self._visualizer.plot_detectron(image[b], target[b], tag=f"{self._mode}_detectron_{c}")
